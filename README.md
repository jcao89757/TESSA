...
The running time of the script was tested on a 256GB node in the UT Southwestern Nucleus Compute CLuster (with 48 logical cores and 256GB memory). Converting the TCRs to embeddings typically spends less than 5s. It took 5.8 min to construct the tessa networks on the 500-cell example data used in this tutorial. For a real dataset contains about 8000 cells, the running time could range to 2 hours. 
